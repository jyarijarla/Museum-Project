  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const res = await fetch(`${API_BASE()}/api/artifacts`, {
        ...
        body: JSON.stringify({ name: form.name.trim(), description: form.description.trim() || null, entryDate: form.entryDate || null, exhibitId: 2, userId: user?.userId || user?.UserID || user?.id })
      })
        ...
        <div className="artifact-modal-overlay" onClick={closeModal}>
          <div className="artifact-modal" onClick={e => e.stopPropagation()}>
            <div className="artifact-modal-header">
              <h2>Add Artifact — Natural History</h2>
            ...
            <form className="artifact-form" onSubmit={handleSubmit}>
              <label className="artifact-label">Name <span className="artifact-required">*</span>
                <input className="artifact-input" type="text" value={form.name} onChange={e => setForm(f => ({...f, name: e.target.value}))} maxLength={120} placeholder="e.g. Triceratops Skull Cast" />
              </label>
